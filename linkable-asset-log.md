...
## 2026-09-13 (new asset: statistics citation hub, backlog item #8)

**Order of work**: built the new asset from backlog item #8 (`Salary Statistics 2026`) rather than a distribution pass — this asset type (statistics citation hub, backlog's 4th linkable-asset category) had no prior published instance on the site, so there was no distribution-saturation check to run first.

**What was built**: `salary-statistics-2026`, curating 61 distinct statistics (13-item Key Takeaways block + 48 across 11 question-form H2 sections) on U.S. pay by education, gender, race, industry, union status, minimum-wage incidence, new-graduate starting salaries, and salary-negotiation sentiment. Structure follows the hub-page spec: Key Takeaways up top, question-style H2s answered in their first sentence, four elements per statistic (number, source, date, original link), a Methodology + Cite-this closer, and a self-made SVG bar chart (median weekly earnings by education level).

**Sources (17, all primary publishers)**: BLS (7 releases: OEWS occupational wages, usual weekly earnings, Employment Cost Index, real earnings, employer costs for compensation, union members, and the Characteristics of Minimum Wage Workers report), the DOL minimum-wage reference page, Census Bureau's Income in the United States: 2024 (P60-286), the Atlanta Fed's Wage Growth Tracker, Robert Half's 2026 Salary Guide, and 6 NACE compensation articles. Two sources named in the original backlog candidate (USAFacts.org, HR Source) were dropped: USAFacts's relevant pages are JS-rendered React shells that curl cannot extract real numbers from (couldn't independently verify any figure sourced from them), and HR Source's public homepage carries no citable statistics, only a membership pitch. Neither drop reduced the asset below the ≥50-statistic bar (61 delivered) or the source-diversity requirement (17 distinct primary publishers across 5 institution types).

**Verification**: every source URL curl-checked 200 with a declared-contact UA (BLS/Census reject a bare browser UA but accept one with a contact email, consistent with this site's existing BLS-fetch convention). Every cited number, date, and USDL/report identifier cross-checked word-for-word against the fetched source text before writing.

**Independent review, round 1**: found 5 real problems, all fixed. (1) The article claimed healthcare was "the highest mean wage of any major group BLS tracks" at $108,700 — the reviewer independently pulled BLS's own `ocwage.t01.htm` data table and found management ($145,260) and legal occupations ($139,510) both pay more nationally; softened to "one of the higher-paying major groups." (2) A claim that petroleum engineering was the "highest projected starting salary among Class of 2026 majors" overreached the source, which only ranked it highest *within engineering*; scope narrowed to match. (3) A "widest weekly-earnings gap by age" ranking claim wasn't supported by the source, which gave only two age-bracket examples, not a full ranking; reworded to describe the widening pattern without the unsupported superlative. (4) A "0.8% to 1.3%" range for minimum-wage incidence conflated a male/female split with an overall estimate; the source's actual overall figure is 1.0%, now stated directly with the gender split noted separately. (5) The draft never linked to the site's existing `psychiatrist-salary` page despite discussing physician-level pay; added one sentence with that internal link.

**Independent review, round 2**: re-verified all 5 fixes against the same primary sources (fresh curl fetches, not reused from round 1) and confirmed each is now accurate and scope-appropriate; also ran an esbuild parse check on the whole `guides.ts` file to confirm the string edits hadn't broken TypeScript syntax. No further issues found.

**Mechanical checks**: `title_lint.py` PASS, `check_seo_field_stats.py` normal range (description) with one flagged-but-justified short title (48-char hard cap from the title-quality skill leaves no room to lengthen it toward the site's mean), `check_prose_patterns.py` PASS (4/4, including one FAQ-rewrite cycle to clear a duplicate-text flag), `check_bridge_sentences.py` 0 candidates (no internal links carry comparison language). `npm test` 92/92, `npm run build` 81 pages, both clean.

**Push held, not skipped**: WageLark is 41 days old (5.9 weeks), inside the 0-6-week ≤1-article/day speed-ladder bracket. Today's regular `wagelark-content-publishing` run already published one article (`speech-language-pathologist-salary`, 13:14). Publishing this asset today as well would be a second new URL from this domain in one day, over that bracket's cap. Committed locally (`97da8b3`) with push held for the next daily run to pick up alongside its own article, logged as a dashboard todo rather than silently deferred.
